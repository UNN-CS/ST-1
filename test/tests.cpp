...
TEST(TestcheckPrime, checkNegative) {
	FAIL();
	//Negative numbers cause incorrect work and it can't be fixed because of objective terms
	EXPECT_EQ(checkPrime(-1), false);
	EXPECT_TRUE(true);
...
TEST(TestNPrime, checkDoubleTwoNHalf) {
	//Double numbers can cause incorrect work and it can't be fixed because of objective terms
