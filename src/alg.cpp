...
void newPrimes(uint64_t n) {
  ...
  for (int i = 2; i < sqrt(n) + 1; i++)  if (tmp[i] != 0)  {
    for (int j = i * i; j < n; j += i) tmp[j] = 0;
  }
  if (n > 2) {
    tmp[0] = 0;
    tmp[1] = 0;
  ...
  for (int i = last_checked; i < n; i++)  if (tmp[i] == 1) {
    primes.push_back(i);
  }
  last_checked = n;
}
...
uint64_t nextPrime(uint64_t value) {
  if (value < 2) return 2;
  while ((last_checked < value) || (primes[primes.size() - 1] <= value)) {
    newPrimes(last_checked + value * 5);
  }
